...
class Classs extends React.Component {
                ...
                },
                {
                    ...
                    renderCell: (params) => {
                        const handleDelete = async (e) => {
                            ...
                            const id = params.id;
                            const classId = api.getCellValue(params.id, 'ClassID');
                            const response = await fetch(`http://localhost:5000/class/delete`, {
                                method: 'POST',
                                headers: {
    ...
    componentDidMount() {
        fetch('http://localhost:5000/class/get-all', {
            method: 'GET', // or 'PUT'
            headers: {
                'Content-Type': 'application/json',
                authorization: TokenService.getLocalAccessToken(),
            ...
            <Container maxWidth="lg">
                <Container maxWidth="lg" sx={{ mt: 5, mb: 10 }}>
                    <div style={{ height: 630, width: '100%' }}>
                        ...
                        />
                        <div id="details-form" />
                        <ClassDialog savechange={this.saveChange} Notify={this.Notify} count={this.state.rows.length} />
                    </div>
                </Container>
