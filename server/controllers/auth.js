...
export const registerAccount = async (req, res) => {
  ...
  try {
    ...
    await newUser.save()
    const accessToken = jwt.sign({ username: username, role: role }, '123456' || process.env.SECRET_KEY)
    return res.json({
      success: true,
...
export const login = async (req, res) => {
  ...
  try {
    ...
    const accessToken = jwt.sign({ userId: user._id }, '123456' || process.env.SECRET_KEY)
    return res.json({
      success: true,
